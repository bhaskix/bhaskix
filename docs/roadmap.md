...
|---|---|---|---|
| **L1** | Static ELF binaries, BusyBox, shell utilities, `curl`, OpenSSH | Its prerequisite is **met**: [RFC 0033](rfc/0033-what-a-hosted-process-is.md), accepted 2026-08-20, gave it a process record, a pid that survives an `execve`, a descriptor table held by the adapter, real `openat`/`read`/`close`/`dup`, `pipe2` with a reader that parks, `fork`, `wait4` and `/proc/self/{status,maps}`. What is left is **breadth, not shape**: terminal `ioctl`s, `getdents`, `stat`, ~~a writable path~~, and ~~the signal gaps a shell notices~~ — **the first of those closed 2026-09-15**, [RFC 0079](rfc/0079-a-signal-a-process-may-send-another.md): `kill(pid, sig)` for `SIGKILL`, `SIGTERM` and the `sig == 0` probe, plus `kill(-pgid, sig)`, with the process tree as the rule for who may signal whom and `ESRCH` for everything outside it. ~~What remains of the signals is **delivery to a handler** — `rt_sigaction` records one and nothing runs it, so `SIGTERM` is fatal here rather than catchable~~ **— delivery landed 2026-09-19, [RFC 0083](rfc/0083-a-signal-a-process-can-catch.md): a hosted process catches `SIGTERM`, whether it sent it to itself or it arrived while the process was parked in a call, and a forked child inherits its parent's handlers as it does on Linux. `SIGKILL` stays uncatchable by construction. The limit is that delivery happens at a call or a fault and nowhere else, so a process making no calls does not receive one — printed on every boot as the difference between signals raised and signals delivered, with a written trigger for closing it.** What remains is `SIGSTOP`/`SIGCONT`, which a domain has no stopped state for, and `sa_mask` during delivery, which is recorded and not enforced. **The writable path closed 2026-09-13** — [RFC 0060](rfc/0060-a-writable-path-for-a-hosted-process.md), all six steps: a hosted process creates a file under `/tmp`, writes to it, closes it, reopens it and reads its own bytes back off a journalled disk, and makes directories and removes files there. It is refused everywhere else structurally rather than by a check, and two of the five boot gates assert that refusal rather than the permission. **It cost no `unsafe`**: `COPY_IN` names an object and an offset rather than an address, so the bytes never enter the adapter's address space and `bin/linuxd`'s exact budget is still 117. So of the five things this row listed, the last one that is a *mechanism* is done and what remains is breadth and signals. ~~**One named gap inside it**: `O_TRUNC` on a file that already exists is silently ignored.~~ **Closed 2026-09-13** — [RFC 0077](rfc/0077-a-file-that-can-be-emptied.md): `dir::TRUNCATE`, badge-gated like every other method that changes something, and a `Volume::truncate` that frees a file's blocks while keeping its inode and generation. `echo x > file` over a longer file leaves nothing of the longer one, and `ftruncate(fd, 0)` works; a non-zero length is refused with `EINVAL` rather than rounded to zero, because this format cannot represent a hole. **`argv` and environment through `execve` are done** — [RFC 0059](rfc/0059-an-execve-that-runs-a-program.md), accepted 2026-08-30: a hosted process execs a real ELF read off the filesystem by a loader in ring 3, with the arguments and environment its parent chose and an auxiliary vector the program follows. **Both of those limits are gone as of 2026-09-01** — [RFC 0064](rfc/0064-a-read-that-lands-where-the-caller-says.md) made the staging object a window a file streams through, and [RFC 0065](rfc/0065-the-block-the-format-already-had.md) made `Volume::write` use the indirect block the format had always carried, taking the file limit from 40,960 bytes to 4,239,360. A 76,992-byte program is stored, read back and `execve`d on every boot with a filesystem, which is larger than both old limits. **What stopped BusyBox next was time, and that too is now measured and mostly removed**: every block of a write was one journal transaction at 14–47 ms, so its 531 blocks would have added 7.5 to 25 seconds to a boot. [RFC 0066](rfc/0066-one-commit-for-many-blocks.md) gives the filesystem `write_run`, one commit for many blocks — a run stages the same three metadata blocks whatever its length, because RFC 0015 never journals data — and the boot report now prices the same write at **3.8–4.4 ms a block**, putting BusyBox at about **2.1 seconds**. What remains between here and a hosted `sh` running a command is a disk and a formatted filesystem large enough to hold it — 1 MiB and 128 blocks today, against the 531 blocks BusyBox needs — **and the boot now prices both halves of what that would cost**. The format writes *every* block of the filesystem, used or not, one block per device call: 128 blocks take **437–1,061 ms** depending on host load, so 600 blocks would be roughly **2–5 seconds**, on top of the **2.0–2.7 s** the staging write would add. **The format is the larger half, which is not where the effort has gone.** Its per-block cost (3.4–8.3 ms) overlaps the journalled staging write's (3.8–5.1 ms), so the transaction RFC 0066 removed was never the whole story: a round trip to the block service dominates both. `block::WRITE` takes a sector *count* in `args[1]`, and the obvious conclusion — that batching therefore needs no protocol change — **is wrong, and was published for an hour before being checked**. `bin/blkd` clamps that count to `ring::SECTORS = 8`, because its ring's data area runs from `0x2800` to the report at `0x3800`: exactly one 4 KiB block, with nothing after it. Batching needs that area to grow and the report to move, on both sides of a layout the kernel and the service share. That is a bounded change and precisely the one RFC 0016 step 3 already made once, from **one** sector to eight — its comment says `args[1]` "had always said how many sectors were wanted and had always been ignored" — so the shape is known and the win is the same order again. It is where the next order of magnitude is, and it would make both halves cheaper at once — specified as [RFC 0067](rfc/0067-more-than-one-block-per-round-trip.md), which names the three parts and says plainly that each is useless alone: raising the sector count by itself gains nothing, because every caller asks for eight | 🔨 **started 2026-08-20; BusyBox runs as of 2026-08-27** — `bin/busybox`, a static BusyBox 1.30.1 nobody here wrote or built, loads through this kernel's own ELF loader into a Linux-tagged domain and prints `hello from busybox` from its `echo` applet in sixteen calls. Ten syscalls were added to reach it, measured from the machine's own trace rather than guessed: `stat`, `lstat`, `getuid`, `geteuid`, `getppid`, `prctl`, `access`, `writev`, `tgkill` and `brk` — plus region splitting in the VM, without which glibc's static-PIE relocation could not re-protect its RELRO. Its **shell** runs too: `sh -c 'echo hi from sh'` prints in twenty-nine calls, gated and watched red. ~~**What that is not**: an *interactive* shell — `sh` reaches a correct `/ #` prompt and cannot read a key.~~ **Stale since 2026-08-27, and corrected here rather than deleted.** Input for a hosted process was a decision about authority and the decision was made: [RFC 0053](rfc/0053-input-a-domain-was-given.md) grants input to a *domain* rather than giving the adapter console `READ`, and RFC 0054, 0055 and 0057 gave it a read that waits, a `poll` that tells the truth and a park naming two wake sources. `tests/qemu/busybox-test.sh` asserts *"a hosted program read a line typed at the machine and ran it"*, and requires the reply to arrive while BusyBox still holds the console so the Bhaskix shell cannot have answered instead. Also no filesystem breadth, and `readlink("/proc/self/exe")` still refused. A static Go binary also loads, runs and traces its calls, and seven hosted probes exercise the process surface. **Two things joined it on 2026-09-04, and neither is "BusyBox runs" — that was already true above.** *(1) BusyBox now runs from the **filesystem**, not the initrd*: 2,172,376 bytes staged onto a disk-sized filesystem and resolved by a hosted `execve` through the adapter's **directory capability**, which is the path a real program would take rather than the kernel's corpus loader placing it. *(2) Its shell **forks and execs a child***: `sh -c '/busybox echo …'` is a path rather than a builtin, so the text is printed by a process the shell created, with the boot's own `linux fork` and `linux wait` lines beneath it. Behind `bhaskix.busybox=1`, off by default, because staging costs about 1.8 s on a lane with a filesystem |
...
| R6 | The design documents reviewed by two people who did not write them | **not met** — Phase 0's own criterion. **The packet is ready** (2026-08-30): [review-guide.md](review-guide.md) sizes the ask at ~3½ hours, scopes it to the design documents rather than the 500,000 words of tracker and RFCs behind them, says what to look for, and gives a findings template. What remains is two people |
