...
| T10 | Resource exhaustion by one domain denying service to others | `ResourceEnvelope` enforced at allocation and scheduling time, not by best effort | 🔨 **partial, and the memory half is mitigated as of 2026-09-19 rather than still open.** The **CPU** half is built and genuinely gated: shares are divided by thread count, every thread is reweighted when one is added, and the gate puts one thread and three threads on the same CPU and checks the shares hold. The **memory** half was *absent* from 2026-08-26, when this cell was corrected from ✅, until [RFC 0082](rfc/0082-a-domains-own-memory-is-its-own.md): `domain::charge_frames`'s only real caller was `shared::create`, so the envelope bounded **shared objects** and not a domain's own address space. It bounds both now — `map_anonymous`, the demand-paging fault and the copy-on-write copy each charge the owning domain and are refused past its cap, on a gate that drives the real allocation paths and was armed red on all four of its assertions. **What keeps this 🔨 and not ✅ is measured and named: page-table frames are still charged to nobody**, and on the boot that accepted the change they were **306 frames against 1,118 charged** — 0.27× the memory the envelope bounds. A domain can still spend uncharged memory that way, bounded by how much address space it maps rather than by its envelope. That is a smaller hole than the one that was here and it is not zero. *This cell read "306 against 182 charged — 1.7×" for an afternoon, off a charged figure that was six times too low: a space's frames were not handed to its owner when it acquired one, so a program's whole initial image went uncharged. The denominator was the bug* |
...
> **The property has its own gate since 2026-09-19** —
> [RFC 0082](rfc/0082-a-domains-own-memory-is-its-own.md), `vm::envelope_self_test`, reported as
> *"a domain's own memory is charged"*. It allocates rather than counting: an eager `map_anonymous`
> past the cap, a lazy region committed page by page through `service_fault` — the function a real
> page fault reaches — and a copy-on-write copy, which is a new frame and must be charged as one.
> Each is required to be refused at the cap, and the teardown is required to give every frame back,
> so the envelope bounds what a domain *holds* rather than what it has ever touched. All four
> assertions were watched red by removing the charge each is about.
>
> The paragraph above is kept rather than rewritten, because the lesson in it outlived the defect
> and is the reason the new gate is shaped the way it is. The old gate stays too, as the unit check
> of the arithmetic it honestly is.
...
| Physical memory | **Bounded as of 2026-09-19, with one gap that has a number on it.** The envelope charges every frame a domain's address space holds and refuses past its cap — [RFC 0082](rfc/0082-a-domains-own-memory-is-its-own.md). This row read *"bounds shared objects only, not a domain's own address space"* until then, and that was true from the day the envelope was written. What it still does not bound is **page-table frames**, which are charged to nobody and held 306 against 1,118 charged on the boot that accepted the change. The per-frame `owner: DomainId` is still never written; the accounting is per domain, not per frame — [memory.md](memory.md) §2 |
