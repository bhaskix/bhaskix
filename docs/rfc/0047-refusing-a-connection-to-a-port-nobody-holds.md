...
| | |
|---|---|
| **Status** | ✅ **ACCEPTED 2026-08-24 — all four steps.** `bin/tcpd` answers a `SYN` for a port no listener holds with a `RST`, per RFC 793 §3.4, and until this it *could not*: `send_entry` had one caller, inside `Action::Emit`, which needs a control block. The arithmetic is `tcp::state::reset_for` in the crate that is fuzzed and `forbid`s `unsafe`, with `closed_arrival` rewritten onto it so the two cannot drift; four host tests, each watched red by its own mutation — and the third also takes down the pre-existing closed-connection test, which is what proves the refactor load-bearing. One boot gate, watched red by deleting the emit: it failed alone. **What acceptance does not claim, stated here rather than left to be inferred: (1) the intermittent this was found while measuring is *not* proven fixed** — 33 boots on an idle machine never reproduced it, so the mechanism is measured and its causal link to the filed row is not; **(2) the gate passes positively on one lane of five** — only `iommu` has networking at all, and the other four take a stated reduction, so the refusal is demonstrated on a single placement; **(3) it has never run on hardware and currently cannot**, there being no NIC this kernel can drive on the only physical machine (see the testing plan's own correction); and **(4) it does not fix the wedged listener** — one `SYN` from a peer that vanishes still takes a listener out, narrowed 242 s → 14 s the same day by [RFC 0048](0048-a-listener-that-cannot-be-wedged.md) step 1 and **not closed**. Unresolved question 1 moved to RFC 0048 and its own TRACKER row; question 2 (a rate limit on resets) stays open with its trigger |
