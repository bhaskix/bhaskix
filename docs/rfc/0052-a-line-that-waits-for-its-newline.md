...
| | |
|---|---|
| **Status** | **Built 2026-08-27, awaiting the project lead.** The console service holds each caller's partial line, keyed by the badge the kernel stamps, and puts it in one `PUT_RUN` when a newline arrives, the buffer fills, or **that caller reads** — the last being what keeps a prompt visible, since a shell writes `bhaskix$ ` with no newline and then waits. Five host tests, three mutations watched red: flushing every write fails three of them, removing the flush-on-read fails **exactly** the prompt test, and ignoring the badge fails the two-callers test. All three shell lanes pass, which is the test that cannot be faked — a wrong flush-on-read hangs every one of them at a prompt nobody can see |
