...
| | |
|---|---|
| **Status** | ✅ **ACCEPTED 2026-08-27** — proposed, built and accepted the same day. A hosted domain can be granted the authority to read what somebody types; `TAKE_INPUT` and `POLL_INPUT` live on the **`Domain`** capability, need `Rights::READ` on it, and the **nucleus** refuses unless the grant names that domain. **The adapter's console capability is untouched — still `WRITE` alone**, and the sentence explaining why is still true word for word. That is the whole reason for this shape: an adapter with console `READ` reads every keystroke for ever, while an adapter with this reads them only for a granted domain and cannot lift the check. **What acceptance does not claim.** *(1)* **No shell has been typed at.** The authority is proved — refused without a grant, exclusive, released with the domain, watched red — and the *keyboard* is not: that needs a lane which types during the corpus, because the boot harness types at the Bhaskix shell and two readers of one keyboard is the muddle this prevents. *(2)* **Nothing is granted on any boot today** except the self-test's own domains, so no hosted program currently reads input, and a compromised adapter reaches no keystroke. *(3)* **`poll` is still not answered by the adapter**: `POLL_INPUT` exists in the nucleus and `bin/linuxd` does not use it, so BusyBox's `sh` still prints `poll: Function not implemented`. *(4)* Unresolved questions 1–3 stay open — who may grant it, whether it can be taken back from a program blocked in a read, and what `poll` should say without a grant |
