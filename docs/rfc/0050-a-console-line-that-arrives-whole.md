...
| | |
|---|---|
| **Status** | **Draft — proposed and built 2026-08-27, with the defect it fixes proven from a preserved log.** A hosted program's console line can be split by any concurrent kernel line, because `bin/linuxd` puts one byte per `INVOKE` and `console::_print` locks per call. This proposes one method — put a *run* of bytes under one lock — and states plainly that it reverses a deliberate choice, which is why it is an RFC rather than a patch. Acceptance is the project lead's |
