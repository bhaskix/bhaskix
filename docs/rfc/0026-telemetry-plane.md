...
| | |
|---|---|
| **Status** | ✅ **Accepted 2026-08-17**, on the working demonstration — drafted, implemented (all six steps) and accepted the same day. What stands: the host-tested arithmetic whose tail clamp was made to fail before it was believed; one drop-newest ring per CPU with the emit path measured from both sides (~1395 cycles enabled, ~247 disabled, TCG); two boot gates on every placement — the report line and the round trip, the latter negative-armed by withholding the grant and watching `bin/traced` fail closed; and five producers streaming the kernel crossings to a live deadline-woken consumer. Step 5 landed differently than sketched and the plan was edited to match *before* acceptance: general `Syscall`-class crossing schemas rather than a `Net`-class TCP re-expression, and the pipeline stamps retired with one loss stated — deliver-to-seen, an in-program memory poll no kernel crossing can see. Questions 3 and 4 were answered by implementation: `bin/traced` ships in every initrd (the VFS listing check counts it), and the blocking reader is a notification with an armed deadline, the shape every service speaks. Questions 1 (per-domain enable bits — theirs is the first consumer that needs them, the audit RFC at latest) and 2 (a flight-recorder mode — one header bit and a tearing-tolerant reader, wanted by the next hang hunt) stay open, each with its trigger written down |
