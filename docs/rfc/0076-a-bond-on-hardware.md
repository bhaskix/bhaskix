...
### The transmit ring never honoured the fetch granularity — 2026-09-10

The two readings offered at the end of the last section were the control-port
hypothesis and the ring. **It is the ring, and the datasheet says so in one
sentence.**
...
`bin/netd` clears PXE mode at bring-up — deliberately, it is what
`38.30.2.1`'s *"operating system driver only"* step asks for — and these are
16-byte descriptors. So the device fetches **eight at a time and will not fetch
part of a line**, while this driver bumped the tail by one for a plain frame and
two for an uplink-tagged one. Every post left a partial line the device would not
touch, which is precisely what the last boot measured: `QTX_HEAD` at 6 against a
cursor of 13, and 32 of 36 uplink posts never written back.

**The receive ring already knew.** `bin/netd`'s own constant reads *"Descriptors
in each receive ring: a whole multiple of 32 outside PXE mode."* The rule was
learned once, written down on one side of the driver, and never applied to the
other.

**The fix, and why it looks wasteful.** A frame now takes a whole cache line: its
one or two descriptors, then NOPs to the boundary, and the tail lands there.
38.31.2.1.2 defines the padding: *"A NOP descriptor can be used if there is a
software need to align descriptors… DTYP should be set to 0x1 (LAN context
descriptor) and all other fields should be cleared. Note that NOP descriptors are
permitted only between commands"* — between packets, which is where these go. The
ring goes from 8 descriptors to **32**, which 38.31.3.4.2 permits (*"at smaller
queue size than 32 descriptors the QLEN must be a whole number of 8
descriptors"*) and which is four lines rather than one.

Eight descriptors per frame is generous with a resource this driver has no use
for: it sends one self-contained frame at a time and waits for it, so four frames
in flight is four more than it has ever needed, and 512 bytes of a 4 KiB page
whose packet buffer starts at 2048. That last fact now has a compile-time
assertion, because it was true by accident while the ring was 128 bytes.

**Watched red.** `every_transmit_post_leaves_the_tail_on_a_fetch_boundary` posts
plain and uplink-tagged frames around the ring and asserts the cursor lands on a
line boundary, the frame sits inside the line the tail closes, and the slack is
NOPs rather than whatever the last pass left. Against the old packing it fails
with *"round 0: a tail of 1 is inside a cache line the device will not fetch"*.

**Two existing tests encoded the old packing and were rewritten**, which is worth
naming rather than doing quietly: `the_transmit_cursor_wraps_instead_of_running_past_the_ring`
asserted *"one descriptor per frame, in order"*, and
`an_uplink_frame_takes_two_contiguous_descriptors` asserted a tail of 2 after the
first pair. Both were true of the code and false of the device. What each test
was *for* is unchanged — the cursor must stay a valid index and come back to
zero, and an uplink pair must be contiguous — and that half is what was kept.

**This does not yet claim the switch will aggregate.** It claims the frames will
be fetched. The next boot is what says whether they reach the wire, and the
control-port reading above stays on the table until it does.
