...
| | |
|---|---|
| **Status** | **Draft — proposed 2026-08-27.** The ring 3 shell cannot answer *"is the keyboard working?"* because the counters that would answer it live in the nucleus and it holds no capability that reads them. One nucleus method and one service message, so the shell asks rather than being told. Acceptance is the project lead's |
