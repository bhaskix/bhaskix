...
//! An ELF64 loader for user programs.
//!
//! `docs/roadmap.md` M6 asks for an ELF64 loader "with a fuzz target". The
//! parsing half lives here and is pure; the mapping half needs an address
//! space and is in [`load_into`].
//!
//! # This is a parser for hostile input, and it is the most dangerous one yet
//!
//! An ELF file names addresses, sizes and offsets, and a loader's whole job is
//! to *act* on them — allocate here, copy that many bytes, mark it executable.
//! A `ustar` header that lies produces a short listing; an ELF header that
//! lies, believed, produces a mapping the attacker chose.
//!
//! So every field is checked against the file it came from and against the
//! address space it would land in, and the checks are stated as refusals
//! rather than clamps. Clamping a segment that runs off the end of the file
//! maps *something* at an address the program will jump to; refusing it does
//! not.
//!
//! # What is deliberately refused
//!
//! - **Anything but `ET_EXEC`.** No shared objects, no PIE, no relocation
//!   processing. A dynamic loader is a much larger attack surface and
//!   [RFC 0005](../../../docs/rfc/0005-linux-abi-compatibility.md) argues the
//!   first target should be static binaries precisely to avoid it.
//! - **Segments in the kernel half.** A user program that could ask to be
//!   mapped at a kernel address would be asking the loader to overwrite the
//!   kernel, and the loader has the authority to do it.
//! - **`p_memsz` below `p_filesz`.** The tail is zero-filled; a memory size
//!   smaller than the file size describes a segment that cannot exist.
//! - **Writable and executable together.** `docs/security.md` calls W^X
//!   structural, and a loader that honoured `PF_W | PF_X` because the file
//!   asked would make it advisory.
//!
//! # What is not here
//!
//! - **No interpreter, no dynamic section, no symbols.** Nothing reads them.
//! - **No `.bss` beyond the zero-fill** each segment declares.
//! - **No stack or heap.** The caller maps those; the loader places segments
//!   and reports an entry point.

use bhaskix_boot::VirtAddr;
use bhaskix_mm::{FRAME_SIZE, Protection, VirtRange};
...
fn page_span(segment: &Segment) -> Option<(u64, u64)> {
    let start = segment.address & !(FRAME_SIZE - 1);
    let end = segment.address.checked_add(segment.memory_size.max(1))?;
    let end = end.checked_next_multiple_of(FRAME_SIZE)?;
    Some((start, end))
}
...
pub fn load_into(
    ...
    for segment in image.segments() {
        ...
        space
            .map_anonymous(range, segment.protection)
        ...
        while copied < segment.file_size {
            ...
            let page = virtual_address & !(FRAME_SIZE - 1);
            let within = (virtual_address - page) as usize;
            let chunk = (FRAME_SIZE as usize - within).min(segment.file_size - copied);
            ...
            unsafe {
                core::ptr::copy_nonoverlapping(
                    source.as_ptr(),
                    (hhdm_base + (physical & !(FRAME_SIZE - 1)) + within as u64) as *mut u8,
                    chunk,
                );
            }
            copied += chunk;
...
mod tests {
    ...
    fn elf(
        ...
        let mut bytes = alloc::vec![0u8; contents_at + filesz as usize];
    ...
    fn two_segments_on_separate_pages_are_accepted() {
        ...
        let protections: alloc::vec::Vec<_> =
            image.segments().map(|segment| segment.protection).collect();
